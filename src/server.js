...
app.get(SALES_ROUTES, async (req, res) => {
  try {
    ...
    const limit = Math.min(50, Math.max(10, Number.parseInt(req.query.limit || '10', 10) || 10));
    const offset = (page - 1) * limit;
    ...
    if (req.query.from) {
      whereClauses.push("DATE(s.created_at) >= DATE(?)");
    ...
    const rows = await dbAll(
      `${getSaleSummaryQuery} ${where} ORDER BY s.id DESC LIMIT ? OFFSET ?`,
      [...params, limit, offset]
    );
    ...
    res.json({
      pagination: {
        ...
        page,
        limit,
        pages: Math.max(1, Math.ceil((countRow?.count ?? 0) / limit)),
      },
      data: rows,
    });
  } catch (error) {
